...
DB: SQLite local (socimi_borme.db)
"""
...
def iter_section_c_items(sumario: Dict[str, Any]) -> Iterable[Tuple[Dict[str, Any], Optional[str]]]:
    """
    Devuelve tuplas (item, apartado_nombre) para la Sección C.
    """
            ...
            if apartados:
                for ap in apartados:
                    ...
                    for item in _as_list(ap.get("item")):
                        yield item, ap_name
            # Y si hay items colgando directamente (por si acaso)
...
def text_from_borme_html(html: str) -> str:
    ...
    # El texto de interés está bajo "#### TEXTO" y/o dentro de <div id="textoxx">.
    # Para simplificar, tomamos todo el texto del artículo.
    main = soup
    # Limpiamos menús
...
def process_date(fecha: dt.date, conn: sqlite3.Connection) -> int:
    ...
    for item, apartado in iter_section_c_items(sumario):
        ...
        if not url_html:
            # Solo filtramos sobre HTML (texto). Si no hay HTML, saltamos.
            continue
...
def cmd_run(args):
    ...
    fecha = today
    # Si hoy es sábado o domingo, intentamos el viernes anterior
    if today.weekday() >= 5:
...
def daterange(d1: dt.date, d2: dt.date) -> Iterable[dt.date]:
